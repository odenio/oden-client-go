...
func (a *MetricGroupsAPIService) V2MetricGroupSearchPostExecute(r ApiV2MetricGroupSearchPostRequest) ([]MetricGroup, *http.Response, error) {
	...
	localVarPath := localBasePath + "/v2/metric_group/search"

	localVarHeaderParams := make(map[string]string)
	localVarQueryParams := url.Values{}
	localVarFormParams := url.Values{}
	if r.metricGroup == nil {
		return localVarReturnValue, nil, reportError("metricGroup is required and must be specified")
	}
	...
	// body params
	localVarPostBody = r.metricGroup
	if r.ctx != nil {
