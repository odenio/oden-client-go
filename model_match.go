...
// Match Describes which item(s) to match when searching for an entity: - `unique` (default): will return an error if the search object matches multiple entities - `all`: will return every item that matches search entity - `last`: will return the most recent object, where applicable - `first`: will return the least recent object, where applicable 
type Match string
...
// All allowed values of Match enum
var AllowedMatchEnumValues = []Match{
	"first",
	"last",
	"unique",
	"all",
...
func (v *Match) UnmarshalJSON(src []byte) error {
	var value string
	err := json.Unmarshal(src, &value)
	if err != nil {
		return err
	}
	enumTypeValue := Match(value)
	for _, existing := range AllowedMatchEnumValues {
	...
	return fmt.Errorf("%+v is not a valid Match", value)
}

// NewMatchFromValue returns a pointer to a valid Match
// for the value passed as argument, or an error if the value passed is not allowed by the enum
func NewMatchFromValue(v string) (*Match, error) {
	ev := Match(v)
	if ev.IsValid() {
		return &ev, nil
	} else {
		return nil, fmt.Errorf("invalid value '%v' for Match: valid values are %v", v, AllowedMatchEnumValues)
	}
}

// IsValid return true if the value is valid for the enum, false otherwise
func (v Match) IsValid() bool {
	for _, existing := range AllowedMatchEnumValues {
		if existing == v {
			return true
		}
...
// Ptr returns reference to Match value
func (v Match) Ptr() *Match {
	return &v
}
