...
type ApiExecuteDashboardRequest struct {
	ctx context.Context
	ApiService *DashboardsAPIService
	dashboardExecuteRequest *DashboardExecuteRequest
}

func (r ApiExecuteDashboardRequest) DashboardExecuteRequest(dashboardExecuteRequest DashboardExecuteRequest) ApiExecuteDashboardRequest {
	r.dashboardExecuteRequest = &dashboardExecuteRequest
	return r
}

func (r ApiExecuteDashboardRequest) Execute() ([]DashboardExecuteResult, *http.Response, error) {
	return r.ApiService.ExecuteDashboardExecute(r)
}

/*
ExecuteDashboard Execute a dashboard
...
- Template variables (`{var_name}` in stored OQL) are not substituted
 ...
 @return ApiExecuteDashboardRequest
*/
func (a *DashboardsAPIService) ExecuteDashboard(ctx context.Context) ApiExecuteDashboardRequest {
	return ApiExecuteDashboardRequest{
		ApiService: a,
		ctx: ctx,
	}
...
//  @return []DashboardExecuteResult
func (a *DashboardsAPIService) ExecuteDashboardExecute(r ApiExecuteDashboardRequest) ([]DashboardExecuteResult, *http.Response, error) {
	var (
		localVarHTTPMethod   = http.MethodPost
		localVarPostBody     interface{}
		formFiles            []formFile
		localVarReturnValue  []DashboardExecuteResult
	)

	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "DashboardsAPIService.ExecuteDashboard")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/v2/dashboard/execute"

	localVarHeaderParams := make(map[string]string)
	localVarQueryParams := url.Values{}
	localVarFormParams := url.Values{}
	if r.dashboardExecuteRequest == nil {
		return localVarReturnValue, nil, reportError("dashboardExecuteRequest is required and must be specified")
	}
	...
	// body params
	localVarPostBody = r.dashboardExecuteRequest
	if r.ctx != nil {
